    ...
    if (items != null && JSON != null) {
        try {
            ...
            for (var i = 0; i < items.length; i++) {
                var item = items[i];
                if (item.sku != null && item.name != null && item.price != null && item.image != null && item.quantity != null) {
                    item = new cartItem(item.sku, item.name, item.price, item.image, item.quantity);
...
shoppingCart.prototype.addItem = function (sku, name, price, image, quantity) {
    ...
    if (quantity != 0) {
        ...
        // new item, add now
        if (!found) {
            var item = new cartItem(sku, name, price, image, quantity);
            this.items.push(item);
        }
...
// items in the cart
//
function cartItem(sku, name, price, image, quantity) {
    this.sku = sku;
    this.name = name;
    this.price = price * 1;
    this.image = image;
